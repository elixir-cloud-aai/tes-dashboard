  ...
  border: 1px solid
    ${(props) => {
      if (props.variant === "primary")
        return props.active ? "#4299e1" : "#4299e1";
      if (props.variant === "success")
        return props.active ? "#38a169" : "#38a169";
...
const NetworkTopology = () => {
  ...
  const loadNetworkTopology = useCallback(async (isInitialLoad = false) => {
    try {
      if (isInitialLoad) {
        setLoading(true);
      }
      ...
      const mergedInstances = locations.map((loc) => {
        const dash =
          (dashboardData.tes_instances || []).find(
            (inst) => inst.url === loc.url,
          ) || {};
        ...
        return {
          ...
          throughput: isHealthy
            ? (dash.throughput ?? "Not Available")
            : "Offline",
      ...
      const batchRuns = Array.isArray(dashboardData.batch_runs)
        ? dashboardData.batch_runs
        : [];
      const workflowRuns = Array.isArray(dashboardData.workflow_runs)
        ? dashboardData.workflow_runs
        : [];
      ...
      const totalStorage = storageLocations.reduce((sum, storage) => {
        ...
        const capacityNum =
          parseFloat(String(storage.capacity).replace(/[^\d.]/g, "")) || 0;
        const unitMultiplier = String(storage.capacity).includes("PB")
          ? 1000
          : 1;
      ...
      setRealTimeData({
        activeInstances: mergedInstances.filter((i) => i.status === "healthy")
          .length,
        totalTasks: mergedInstances.reduce(
          (sum, inst) => sum + (inst.taskCount || 0),
          0,
        ),
      ...
      setNetworkStatus(
        mergedInstances.every((i) => i.status === "healthy")
          ? "success"
          : "warning",
      );
    } catch (err) {
  ...
  const generateDataTransfers = (instances, storage) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      transfers.push({
        id: `transfer-${i}`,
        source: {
          lat: sourceInstance.lat,
          lng: sourceInstance.lng,
          name: sourceInstance.name,
        },
        target: {
          lat: targetStorage.lat,
          lng: targetStorage.lng,
  ...
  const filteredInstances = instances.filter((instance) => {
    ...
    const matchesRegion =
      filterRegion === "all" || instance.region === filterRegion;
      ...
      <div style={{ padding: "16px" }} className="p-10">
        <QuickStats>
          ...
          <div className="stat-item">
            ...
            <div>
              <div className="value">
                {realTimeData.totalStorage.toFixed(1)}TB
              </div>
        ...
        <MapContainerStyled>
          {currentView === "map" ? (
            ...
            >
                  ...
                  >
                    <Popup>
                      <div style={{ minWidth: "250px" }}>
                        ...
                        >
                          <div>
                            <strong>Tasks:</strong> {instance.taskCount}
                          </div>
                          <div>
                            <strong>CPU:</strong> {instance.cpuUsage}%
                          </div>
                          <div>
                            <strong>Region:</strong> {instance.region}
                          </div>
                          <div>
                            <strong>Memory:</strong> {instance.memoryUsage}%
                          </div>
                          <div>
                            <strong>Latency:</strong> {instance.latency}ms
                          </div>
                          <div>
                            <strong>Uptime:</strong> {instance.uptime}
                          </div>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          ...
                          <div>
                            <strong>Throughput:</strong> {instance.throughput}
                          </div>
                        </div>
                        <div
                          style={{
              ...
              {selectedWorkflow &&
                (() => {
                  const workflow = workflowPaths.find(
                  ...
                  if (workflow && sourceInstance && targetInstance) {
                    return (
                      <Polyline
                        ...
                        opacity={0.8}
                        dashArray={
                          workflow.status === "RUNNING" ? "10, 10" : undefined
                        }
                      />
                    );
                  }
          ...
          <MapOverlay>
            ...
            <div className="overlay-stats">
              ...
              <div className="stat">
                ...
                <span className="value">
                  {instances.filter((i) => i.status === "processing").length}
                </span>
              </div>
              <div className="stat">
                <span className="label">Offline</span>
                <span className="value">
                  {instances.filter((i) => i.status === "unhealthy").length}
                </span>
              </div>
              <div className="stat">
                  ...
                  <div className="metric-details">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">
                        {realTimeData.activeInstances}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span>Total Tasks</span>
                      <span className="detail-value">
                        {realTimeData.totalTasks}
                      </span>
                    </div>
                    <div className="detail-item">
                      <span>Running Workflows</span>
                      <span className="detail-value">
                        {realTimeData.totalWorkflows}
                      </span>
                    </div>
                    <div className="detail-item">
                  ...
                  <div className="metric-details">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">
                        {instances.length > 0
                          ? Math.round(
                              instances.reduce((sum, i) => sum + i.latency, 0) /
                                instances.length,
                            )
                          : 0}
                        ms
