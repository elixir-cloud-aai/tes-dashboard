...
const Tooltip = styled.div<{ show: boolean; x: number; y: number }>`
  ...
  transform: ${(props) =>
    props.show ? "translateY(-10px)" : "translateY(0px)"};
...
const TES_LOCATIONS = [
  ...
  {
    id: "tesk-cz-prod",
    name: "TESK/Kubernetes @ ELIXIR-CZ (Prod)",
    country: "Czech Republic",
    status: "healthy" as const,
    x: 65,
    y: 35,
  ...
  {
    id: "tesk-cz-na",
    name: "TESK/Kubernetes @ ELIXIR-CZ (NA)",
    country: "Czech Republic",
    ...
    description: "TESK North America",
    coordinates: { lat: 50.0755, lng: 14.4378 },
  ...
  {
    id: "tesk-de",
    name: "TESK/Kubernetes @ ELIXIR-DE",
    country: "Germany",
    status: "healthy" as const,
    x: 60,
    y: 30,
    tasks: 0,
    workflows: 0,
    url: "https://tesk.elixir-cloud.bi.denbi.de",
  ...
  },
  {
    ...
    description: "TESK instance running on CSC OpenShift",
    coordinates: { lat: 60.1699, lng: 24.9384 },
    capacity: { cpu: 800, memory: "1.5TB", storage: "8TB" },
    version: "1.0",
  },
  {
    ...
    capacity: { cpu: 4, memory: "16GB", storage: "500GB" },
    version: "1.0",
  },
];
...
const NetworkTopologyPage: React.FC = () => {
  ...
  const [workflowExecutions, setWorkflowExecutions] = useState<
    WorkflowExecution[]
  >([]);
  const [dynamicInstances, setDynamicInstances] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchInstanceStatus = async () => {
      try {
        ...
        setDynamicInstances(instancesData);
      } catch (error) {
        console.error("Failed to load instance status for topology:", error);
      }
    };
  ...
  useEffect(() => {
    ...
    const executions: WorkflowExecution[] = dashboardData.batch_runs
      .slice(0, 10)
      .map((run: any, index: number) => ({
        id: run.run_id || `workflow-${index}`,
        type: (run.workflow_type || "Nextflow").toUpperCase() as
          | "CWL"
          | "Nextflow"
          | "Snakemake",
        ...
        steps: generateWorkflowSteps(
          run.workflow_type || "Nextflow",
          index < 3 ? "RUNNING" : run.status || "SUBMITTED",
        ),
      }));
  ...
  const generateWorkflowSteps = (
    ...
    const stepTemplates: Record<string, string[]> = {
      ...
      CWL: ["Preprocessing", "Analysis", "Validation", "Report Generation"],
      Snakemake: ["Data Import", "Processing", "Validation", "Export"],
    };
  ...
  const filteredInstances = TES_LOCATIONS.map((loc) => {
    const dynamic = dynamicInstances.find(
      (d) => d.url?.rstrip?.("/") === loc.url?.rstrip?.("/") || d.id === loc.id,
    );
    return dynamic
      ? {
          ...loc,
          status: dynamic.status === "healthy" ? "healthy" : "unhealthy",
          tasks: dynamic.tasks || 0,
          version: dynamic.version || loc.version,
  ...
  const filteredWorkflows = workflowExecutions.filter(
    (workflow) =>
  ...
  const getInstanceStatus = (
    instanceId: string,
  ): "healthy" | "unhealthy" | "processing" => {
    const runningWorkflows = workflowExecutions.filter(
      (w) =>
        w.path.includes(instanceId) &&
        (w.status === "RUNNING" || w.status === "SUBMITTED"),
    );

    if (runningWorkflows.length > 0) return "processing";

    const instance = filteredInstances.find((loc) => loc.id === instanceId);
    return (instance?.status as "healthy" | "unhealthy") || "unhealthy";
  };
    ...
    <PageContainer>
      <HeaderSection>
        <div>
          ...
          <p>
            Real-time visualization of TES instances and workflow execution
            paths
          </p>
        </div>
        <Controls>
            ...
            >
              ...
              {filteredInstances.map((instance) => {
                ...
                return (
                  <Marker
                    key={instance.id}
                    position={[
                      instance.coordinates.lat,
                      instance.coordinates.lng,
                    ]}
                    ...
                    <Popup>
                      <div style={{ minWidth: "200px" }}>
                        ...
                        <div style={{ fontSize: "14px", lineHeight: "1.4" }}>
                          <div>
                            <strong>Location:</strong> {instance.country}
                          </div>
                          <div>
                          ...
                          <div>
                            <strong>Active Tasks:</strong> {instance.tasks}
                          </div>
                          <div>
                            <strong>Workflows:</strong> {instance.workflows}
                          </div>
                          <div>
                            <strong>Capacity:</strong> {instance.capacity?.cpu}{" "}
                            CPU, {instance.capacity?.memory}
                          </div>
                          <div>
              ...
              {showDataFlow &&
                STORAGE_LOCATIONS.map((storage) => (
                  <Marker
                    key={storage.id}
                    position={[
                      storage.coordinates.lat,
                      storage.coordinates.lng,
                    ]}
                    ...
                    <Popup>
                      <div style={{ minWidth: "180px" }}>
                        ...
                        <div style={{ fontSize: "14px", lineHeight: "1.4" }}>
                          ...
                          <div>
                            <strong>Usage:</strong> {storage.usage}%
                          </div>
                          <div
                            style={{
                              background: "#f7fafc",
              ...
              {showDataFlow &&
                workflowExecutions
                  ...
                  .slice(0, 3)
                  .map((workflow) => {
                    return workflow.storageLocations?.map(
                      (storageId, index) => {
                        const storage = STORAGE_LOCATIONS.find(
                          (s) => s.id === storageId,
                        );
                        const tesInstance = TES_LOCATIONS.find(
                          (t) => t.id === workflow.tesInstance,
                        );
                        ...
                        return (
                          <Polyline
                            ...
                            positions={[
                              ...
                              [
                                tesInstance.coordinates.lat,
                                tesInstance.coordinates.lng,
                              ],
                            ]}
                            pathOptions={{
                              color: "#805ad5",
              ...
              {showWorkflowPaths &&
                workflowExecutions
                  .filter(
                    (workflow) =>
                      workflow.status === "RUNNING" && workflow.path.length > 2,
                  )
                  .slice(0, 2)
                  .map((workflow) => {
                    const connections = [];
                    for (let i = 0; i < workflow.path.length - 1; i++) {
                      const fromInstance = TES_LOCATIONS.find(
                        (loc) => loc.id === workflow.path[i],
                      );
                      const toInstance = TES_LOCATIONS.find(
                        (loc) => loc.id === workflow.path[i + 1],
                      );
                      ...
                      if (fromInstance && toInstance) {
                        connections.push(
                          <Polyline
                            ...
                            positions={[
                              [
                                fromInstance.coordinates.lat,
                                fromInstance.coordinates.lng,
                              ],
                              [
                                toInstance.coordinates.lat,
                                toInstance.coordinates.lng,
                              ],
                            ]}
                            pathOptions={{
                              color: "#f56565",
              ...
              {showWorkflowPaths &&
                workflowExecutions
                  .filter(
                    (workflow) =>
                      !selectedWorkflow || workflow.id === selectedWorkflow,
                  )
                  .map((workflow) => {
                    const pathInstances = workflow.path
                      .map((id) => TES_LOCATIONS.find((loc) => loc.id === id))
                ...
                <>
                  {workflowExecutions
                    ...
                    ?.storageLocations.map((storageId) => {
                      ...
                      const workflow = workflowExecutions.find(
                  ...
                  {filteredWorkflows.map((workflow) => (
                    ...
                    >
                      ...
                      <WorkflowDetails>
                        ...
                        <div>
                          Storage: {workflow.storageLocations.length} locations
                        </div>
                        <div>
                  ...
                  <InstanceCard>
                    ...
                    <InstanceDetails>
                      ...
                      <div>
                        ...
                        <span>
                          {
                            TES_LOCATIONS.filter(
                              (loc) => getInstanceStatus(loc.id) === "healthy",
                            ).length
                          }
                        </span>
                      </div>
                        ...
                        <span>
                          {
                            TES_LOCATIONS.filter(
                              (loc) =>
                                getInstanceStatus(loc.id) === "processing",
                            ).length
                          }
                        </span>
                      </div>
                      ...
                      <div>
                        ...
                        <span>
                          {TES_LOCATIONS.reduce(
                            (sum, loc) => sum + loc.tasks,
                            0,
                          )}
                        </span>
                      </div>
                        ...
                        <span>
                          {STORAGE_LOCATIONS.reduce((sum, storage) => {
                            const capacity = parseFloat(
                              storage.capacity.replace(/[^0-9.]/g, ""),
                            );
                            const unit = storage.capacity.includes("PB")
                              ? 1000
                              : 1;
          ...
          <>
            ...
            <TooltipContent>
              ...
              <div className="metric">
                ...
                <span
                  className={`status ${getInstanceStatus(tooltip.data.id)}`}
                >
              ...
              {tooltip.data.coordinates && (
                <div className="coordinates">
                  📍 {tooltip.data.coordinates.lat}°N,{" "}
                  {tooltip.data.coordinates.lng}°E
                </div>
              )}
            </TooltipContent>
          </>
            ...
            <TooltipContent>
              ...
              >
                <div
                  style={{
                    background:
                      tooltip.data.usage > 80
                        ? "#f56565"
                        : tooltip.data.usage > 60
                          ? "#ed8936"
                          : "#48bb78",
                    height: "100%",
