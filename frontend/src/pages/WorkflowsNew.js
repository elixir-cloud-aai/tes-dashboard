...
import { Play, RefreshCw, Eye, FileText, Code, Activity, MapPin } from 'lucide-react';
...
const Workflows = () => {
  ...
  const handleSubmitWorkflow = async (e) => {
    ...
    if (selectedInstances === 'all' && instances.length < 2) {
      setError('Need at least 2 healthy TES instances for distributed execution. Please check instance status.');
      return;
    }
    ...
    try {
      ...
      const snakefileBlob = new Blob([EXAMPLE_SNAKEFILE], { type: 'text/plain' });
      ...
      alert('Workflow submitted successfully! View execution on Network Topology.');
      loadWorkflowRuns();
  ...
  return (
    <WorkflowsContainer>
      <Header>
        ...
        <Subtitle>Execute distributed Snakemake workflows across multiple TES instances</Subtitle>
      </Header>
      ...
      <WorkflowSection>
        ...
        <InfoBox>
          <strong>📊 Distributed Execution:</strong> This example workflow contains 4 steps that will be distributed 
          across your available TES instances. Watch the real-time execution on the Network Topology page!
        </InfoBox>
              ...
              {workflowRuns.map((run, index) => (
                <TableRow key={run.run_id || index}>
                  ...
                  <TableCell>
                    {run.run_id && (
                      <>
                        <ActionButton onClick={() => handleViewLogs(run.run_id)}>
                          <FileText size={12} />
                          Logs
                        </ActionButton>
                        <ActionButton onClick={() => handleViewOnMap(run.run_id)}>
                          <Eye size={12} />
                          View on Map
                        </ActionButton>
                      </>
