...
from services.task_service import update_single_task_status
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            sync_task = {
                                ...
                                'id': task_id,
                                'tes_url': tes_url,
                                'tes_name': instance_name
                            }
                            import threading
                            threading.Thread(target=update_single_task_status, args=(sync_task,), daemon=True).start()
                        except Exception as sync_err:
                            print(f"⚠️ Failed to trigger background sync: {sync_err}")
